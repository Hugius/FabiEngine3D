...
const vector<string> AudioEditor::getAllAudioPathsFromFile()
{
	...
	string filePath = _fe3d.misc_getRootDirectory() + (_fe3d.engine_isGameExported() ? "" : ("projects\\" + _currentProjectID)) + "\\entities\\audio.fe3d";
...
void AudioEditor::saveAudioEntitiesToFile()
{
	if (_isEditorLoaded)
	{
		...
		std::ofstream file;
		file.open(_fe3d.misc_getRootDirectory() + (_fe3d.engine_isGameExported() ? "" : ("projects\\" + _currentProjectID)) + "\\entities\\audio.fe3d");
