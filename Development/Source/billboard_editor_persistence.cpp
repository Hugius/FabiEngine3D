...
void BillboardEditor::loadBillboardEntitiesFromFile()
{
	...
	if (_fe3d.misc_isFileExisting(filePath))
	{
		...
		while (std::getline(file, line))
		{
			...
			{
				_fe3d.billboardEntity_add(billboardID, diffuseMapPath, _billboardPosition + Vec3(0.0f, size.y / 2.0f, 0.0f), 
					Vec3(0.0f), size, transparent, facingX, facingY, false);
			...
			{
				_fe3d.billboardEntity_add(billboardID, textContent, fontPath, color, _billboardPosition + Vec3(0.0f, size.y / 2.0f, 0.0f),
					Vec3(0.0f), size, facingX, facingY, false);
			}
			else // Colored billboard
			{
				_fe3d.billboardEntity_add(billboardID, color, _billboardPosition + Vec3(0.0f, size.y / 2.0f, 0.0f),
					Vec3(0.0f), size, facingX, facingY, false);
			}
