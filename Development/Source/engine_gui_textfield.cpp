...
EngineGuiTextfield::EngineGuiTextfield(FabiEngine3D& fe3d, const string& parentID, const string& ID, vec2 position, vec2 size, 
	string textContent, vec3 textColor, bool isCentered) :
	_fe3d(fe3d),
	_ID(ID),
...
{
	_fe3d.textEntity_add(_entityID, textContent, "engine\\fonts\\lucida.ttf", textColor, position, 0.0f, size, isCentered);
}
