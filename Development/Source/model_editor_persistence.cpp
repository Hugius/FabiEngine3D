...
const vector<string> ModelEditor::getAllTexturePathsFromFile()
{
	...
	string filePath = _fe3d.misc_getRootDirectory() + (_fe3d.engine_isGameExported() ? "" : ("projects\\" + _currentProjectID)) + "\\entities\\model.fe3d";
...
void ModelEditor::saveGameEntitiesToFile()
{
	if (_isEditorLoaded)
	{
		...
		std::ofstream file;
		file.open(_fe3d.misc_getRootDirectory() + (_fe3d.engine_isGameExported() ? "" : ("projects\\" + _currentProjectID)) + "\\entities\\model.fe3d");
