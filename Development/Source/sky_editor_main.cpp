...
void SkyEditor::load()
{
	...
	// Miscellaneous
	_gui.getGlobalScreen()->createTextfield("selectedSkyID", Vec2(0.0f, 0.85f), Vec2(0.5f, 0.1f), "", Vec3(1.0f));
...
void SkyEditor::unload()
{
	...
	// Miscellaneous
	_gui.getGlobalScreen()->deleteTextfield("selectedSkyID");
...
void SkyEditor::_loadGUI()
{
	...
	leftWindow->getScreen("skyEditorMenuMain")->createButton("add", Vec2(0.0f, positions[0]), Vec2(TW("Create Sky"), CH), LVPC::BUTTON_COLOR, LVPC::BUTTON_HOVER_COLOR, "Create Sky", LVPC::TEXT_COLOR, LVPC::TEXT_HOVER_COLOR);
