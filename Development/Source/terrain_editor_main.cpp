...
void TerrainEditor::load()
{
	...
	// Miscellaneous
	_gui.getGlobalScreen()->createTextfield("selectedTerrainID", Vec2(0.0f, 0.85f), Vec2(0.5f, 0.1f), "", Vec3(1.0f));
...
void TerrainEditor::unload()
{
	...
	// Miscellaneous
	_gui.getGlobalScreen()->deleteTextfield("selectedTerrainID");
...
void TerrainEditor::_loadGUI()
{
	...
	leftWindow->getScreen("terrainEditorMenuMain")->createButton("add", Vec2(0.0f, positions[0]), Vec2(TW("Create Terrain"), CH), LVPC::BUTTON_COLOR, LVPC::BUTTON_HOVER_COLOR, "Create Terrain", LVPC::TEXT_COLOR, LVPC::TEXT_HOVER_COLOR);
