...
layout(location = 3) uniform sampler2D   u_sampler_screenReflectionMap;
...
uniform bool u_screenReflectionsEnabled;
...
vec3 applyScreenReflections(vec3 color);
...
void main()
{
	...
	color  = applyScreenReflections(color);
...
vec3 applyScreenReflections(vec3 color)
{
	if(u_screenReflectionsEnabled && u_isScreenReflective)
	{
		vec2 ndc = (f_clip.xy / f_clip.w) / 2.0 + 0.5;
		vec2 texCoords = vec2(ndc.x, -ndc.y);
		vec4 reflectionColor = texture(u_sampler_screenReflectionMap, vec2(texCoords.x,  texCoords.y));
		vec3 mixedColor   = mix(color.rgb, reflectionColor.rgb, u_screenReflectionFactor);
		return mixedColor.rgb;
	}
