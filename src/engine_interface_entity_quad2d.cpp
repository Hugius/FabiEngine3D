...
{
	_core->getQuad2dManager()->createEntity(quad2dId, isCentered);
}
...
void EngineInterface::quad2d_delete(const string & quad2dId)
{
	...
	_core->getQuad2dManager()->deleteEntity(quad2dId);
}

void EngineInterface::quad2d_setVisible(const string & quad2dId, bool value)
{
	_core->getQuad2dManager()->getEntity(quad2dId)->setVisible(value);
}
...
void EngineInterface::quad2d_setDiffuseMap(const string & quad2dId, const string & value)
{
	if(value.empty())
	{
		_core->getQuad2dManager()->getEntity(quad2dId)->setDiffuseMap(nullptr);
		_core->getQuad2dManager()->getEntity(quad2dId)->setDiffuseMapPath("");
	}
	else
	{
...
const vector<string> EngineInterface::quad2d_getIds() const
{
	...
	for(const auto & [quad2dId, quad2d] : _core->getQuad2dManager()->getEntities())
	{
...
{
	return _core->getQuad2dManager()->getEntity(quad2dId)->isHorizontallyFlipped();
}

const bool EngineInterface::quad2d_isVerticallyFlipped(const string & quad2dId) const
{
	return _core->getQuad2dManager()->getEntity(quad2dId)->isVerticallyFlipped();
}