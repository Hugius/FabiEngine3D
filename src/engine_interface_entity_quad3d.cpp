...
{
	_core->getQuad3dManager()->createEntity(quad3dId, isCentered);
}
...
void EngineInterface::quad3d_setDiffuseMap(const string & quad3dId, const string & value)
{
	if(value.empty())
	{
		_core->getQuad3dManager()->getEntity(quad3dId)->setDiffuseMap(nullptr);
		_core->getQuad3dManager()->getEntity(quad3dId)->setDiffuseMapPath("");
	}
	else
	{
		...
		_core->getQuad3dManager()->getEntity(quad3dId)->setDiffuseMap(texture);
		_core->getQuad3dManager()->getEntity(quad3dId)->setDiffuseMapPath(value);
	}
}
...
void EngineInterface::quad3d_setEmissionMap(const string & quad3dId, const string & value)
{
	if(value.empty())
	{
		_core->getQuad3dManager()->getEntity(quad3dId)->setEmissionMap(nullptr);
		_core->getQuad3dManager()->getEntity(quad3dId)->setEmissionMapPath("");
	}
	else
	{
		auto texture = _core->getTextureBufferCache()->get2dBuffer(value);
...
const vector<string> EngineInterface::quad3d_getIds() const
{
	...
	for(const auto & [quad3dId, quad3d] : _core->getQuad3dManager()->getEntities())
	{
...
{
	return _core->getQuad3dManager()->getEntity(quad3dId)->isHorizontallyFlipped();
}

const bool EngineInterface::quad3d_isVerticallyFlipped(const string & quad3dId) const
{
	return _core->getQuad3dManager()->getEntity(quad3dId)->isVerticallyFlipped();
}

const DirectionOrderType EngineInterface::quad3d_getRotationOrder(const string & quad3dId) const
{
	return _core->getQuad3dManager()->getEntity(quad3dId)->getRotationOrder();
}

const bool EngineInterface::quad3d_isWireframed(const string & quad3dId) const
{
	return _core->getQuad3dManager()->getEntity(quad3dId)->isWireframed();
}