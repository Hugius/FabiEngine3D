...
void GuiScrollingList::_updateScrolling()
{
	if(!_buttons.empty())
	{
		if(_isHovered)
		{
			_scrollingOffset += ((_fe3d->input_isMouseScrolled(MouseWheelType::WHEEL_UP) ? 1.0f : _fe3d->input_isMouseScrolled(MouseWheelType::WHEEL_DOWN) ? -1.0f : 0.0f) * _scrollingSpeed);
		}
