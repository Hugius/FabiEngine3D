...
void ModelEditor::_updateTexturingMenu()
{
	...
	if(screen->getId() == "modelEditorMenuTexturing")
	{
		...
		else if(_fe3d->input_isMousePressed(MouseButton::BUTTON_LEFT) && screen->getButton("specularMap")->isHovered())
		{
			if(getCurrentProjectId().empty())
			{
				abort();
		...
		else if(_fe3d->input_isMousePressed(MouseButton::BUTTON_LEFT) && screen->getButton("reflectionMap")->isHovered())
		{
			if(getCurrentProjectId().empty())
			{
				abort();
