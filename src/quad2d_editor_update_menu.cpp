...
void Quad2dEditor::_updateMainMenu()
{
	...
	if(screen->getId() == "quad2dEditorMenuMain")
	{
		...
		{
			_gui->getOverlay()->enableValueForm("quadCreate", "Create Quad2D", "", fvec2(0.0f, 0.1f), fvec2(0.5f, 0.1f), fvec2(0.0f, 0.1f));
		...
		else if(_fe3d->input_isMousePressed(InputType::MOUSE_BUTTON_LEFT) && screen->getButton("edit")->isHovered())
		{
			...
			_gui->getOverlay()->enableChoiceForm("quadList", "Edit Quad2D", fvec2(-0.5f, 0.1f), ids);
		...
		else if(_fe3d->input_isMousePressed(InputType::MOUSE_BUTTON_LEFT) && screen->getButton("delete")->isHovered())
		{
			...
			_gui->getOverlay()->enableChoiceForm("quadList", "Delete Quad2D", fvec2(-0.5f, 0.1f), ids);
			_isChoosingQuad = true;
...
void Quad2dEditor::_updateChoiceMenu()
{
	...
	if(screen->getId() == "quad2dEditorMenuChoice")
	{
		...
		{
			_gui->getOverlay()->enableValueForm("opacity", "Opacity", (opacity * 100.0f), fvec2(0.0f, 0.1f), fvec2(0.15f, 0.1f), fvec2(0.0f, 0.1f));
		}
		else if(_fe3d->input_isMousePressed(InputType::MOUSE_BUTTON_LEFT) && screen->getButton("textureRepeat")->isHovered())
		{
			_gui->getOverlay()->enableValueForm("textureRepeat", "Texture Repeat", textureRepeat, fvec2(0.0f, 0.1f), fvec2(0.15f, 0.1f), fvec2(0.0f, 0.1f));
		}
