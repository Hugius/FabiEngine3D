...
const bool Quad3dEditor::saveEntitiesToFile() const
{
	...
	for(const auto & quadId : _loadedEntityIds)
	{
		...
		auto isHorizontallyFlipped = _fe3d->quad3d_isHorizontallyFlipped(quadId);
		auto isVerticallyFlipped = _fe3d->quad3d_isVerticallyFlipped(quadId);
		...
		auto rotationOrder = static_cast<unsigned int>(_fe3d->quad3d_getRotationOrder(quadId));
