...
void Quad3dEditor::_updateMiscellaneousMenu()
{
	...
	if(screen->getId() == "quad3dEditorMenuMiscellaneous")
	{
		...
		const auto minAlpha = _fe3d->quad3d_getMinAlpha(_currentQuad3dId);
		...
		if((_gui->getOverlay()->getValueFormId() == "sizeX") && _gui->getOverlay()->isValueFormConfirmed())
		{
			...
			_fe3d->quad3d_setSize(_currentQuad3dId, fvec2((value / SIZE_FACTOR), size.y));
		}
		else if((_gui->getOverlay()->getValueFormId() == "sizeY") && _gui->getOverlay()->isValueFormConfirmed())
		{
			...
			_fe3d->quad3d_setSize(_currentQuad3dId, fvec2(size.x, (value / SIZE_FACTOR)));
		}
		else if((_gui->getOverlay()->getValueFormId() == "opacity") && _gui->getOverlay()->isValueFormConfirmed())
		{
			...
			_fe3d->quad3d_setOpacity(_currentQuad3dId, (value / OPACITY_FACTOR));
		}
		else if((_gui->getOverlay()->getValueFormId() == "minAlpha") && _gui->getOverlay()->isValueFormConfirmed())
		{
			...
			_fe3d->quad3d_setMinAlpha(_currentQuad3dId, (value / MIN_ALPHA_FACTOR));
		}
