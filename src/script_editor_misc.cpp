...
void ScriptEditor::_reloadScriptTextDisplay(bool reloadAabbs)
{
	...
	fvec3 separatorPosition = SCRIPT_TEXT_STARTING_POSITION + fvec3(HORIZONTAL_LINE_OFFSET * 0.5f, -(((lineCount - 1) * 0.5f) * VERTICAL_LINE_OFFSET), 0.0f);
	...
	for(unsigned int lineIndex = 0; lineIndex < lineCount; lineIndex++)
	{
		...
		const fvec3 lineAabbPosition = (lineNumberPosition - fvec3(0.0f, TEXT_CHARACTER_SIZE.y * 0.5f, AABB_DEPTH));
		...
		for(unsigned int charIndex = 0; charIndex < lineTextString.size(); charIndex++)
		{
			...
			if(reloadAabbs)
			{
				const fvec3 characterAabbPosition = (characterPosition - fvec3(0.0f, TEXT_CHARACTER_SIZE.y * 0.5f, 0.0f));
