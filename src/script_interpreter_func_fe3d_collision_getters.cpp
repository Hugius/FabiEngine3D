...
const bool ScriptInterpreter::_executeFe3dCollisionGetterFunction(const string& functionName, vector<ScriptValue>& arguments, vector<ScriptValue>& returnValues)
{
		...
		if(_validateArgumentCount(arguments, static_cast<unsigned int>(types.size())) && _validateArgumentTypes(arguments, types))
		{
			...
			if(_validateFe3dAabb(arguments[0].getString()))
			{
				...
				if(_fe3d.aabb_isExisting(arguments[0].getString()))
				{
					...
					if(!_fe3d.aabb_hasParent(arguments[0].getString()))
					{
						// Check direction
	...
	else if(functionName == "fe3d:collision_check_model_models")
	{
		...
		if(_validateArgumentCount(arguments, static_cast<unsigned int>(types.size())) && _validateArgumentTypes(arguments, types))
		{
			...
			{
				// Get part IDs
				for(const auto& selfSearchID : _fe3d.aabb_getChildIDs(arguments[0].getString(), AabbParentType::MODEL_ENTITY))
				{
					// Find aabbEntity ID
	...
	else if(functionName == "fe3d:collision_check_model_billboards")
	{
		...
		if(_validateArgumentCount(arguments, static_cast<unsigned int>(types.size())) && _validateArgumentTypes(arguments, types))
		{
			...
			{
				// Get part IDs
				for(const auto& selfSearchID : _fe3d.aabb_getChildIDs(arguments[0].getString(), AabbParentType::BILLBOARD_ENTITY))
				{
					// Find aabbEntity ID
