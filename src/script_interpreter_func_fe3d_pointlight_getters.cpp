...
const bool ScriptInterpreter::_executeFe3dPointlightGetterFunction(const string& functionName, vector<ScriptValue>& arguments, vector<ScriptValue>& returnValues)
{
	...
	else if(functionName == "fe3d:pointlight_get_position")
	{
		...
		if(_validateListValueCount(arguments, static_cast<unsigned int>(types.size())) && _validateListValueTypes(arguments, types))
		{
			...
			if(_validateFe3dPointlight(arguments[0].getString()))
			{
				auto result = _fe3d.pointlight_getPosition(arguments[0].getString());
	...
	else if(functionName == "fe3d:pointlight_get_radius")
	{
		...
		if(_validateListValueCount(arguments, static_cast<unsigned int>(types.size())) && _validateListValueTypes(arguments, types))
		{
			...
			if(_validateFe3dPointlight(arguments[0].getString()))
			{
				auto result = _fe3d.pointlight_getRadius(arguments[0].getString());
	...
	else if(functionName == "fe3d:pointlight_get_color")
	{
		...
		if(_validateListValueCount(arguments, static_cast<unsigned int>(types.size())) && _validateListValueTypes(arguments, types))
		{
			...
			if(_validateFe3dPointlight(arguments[0].getString()))
			{
				auto result = _fe3d.pointlight_getColor(arguments[0].getString());
				returnValues.push_back(ScriptValue(_fe3d, SVT::VEC3, result));
			}
		}
	}
