...
const bool ScriptInterpreter::_executeFe3dText2dGetter(const string& functionName, const vector<shared_ptr<ScriptValue>>& args, vector<shared_ptr<ScriptValue>>& returnValues)
{
	...
	else if(functionName == "fe3d:text2d_get_position_x")
	{
		...
		if(_validateArgumentCount(args, static_cast<unsigned int>(types.size())) && _validateArgumentTypes(args, types))
		{
			if(_validateFe3dText2d(args[0]->getString(), false))
			{
				const auto result = Tools::convertPositionRelativeFromViewport(_fe3d->text2d_getPosition(args[0]->getString())).x;
	...
	else if(functionName == "fe3d:text2d_get_position_y")
	{
		...
		if(_validateArgumentCount(args, static_cast<unsigned int>(types.size())) && _validateArgumentTypes(args, types))
		{
			if(_validateFe3dText2d(args[0]->getString(), false))
			{
				const auto result = Tools::convertPositionRelativeFromViewport(_fe3d->text2d_getPosition(args[0]->getString())).y;
	...
	else if(functionName == "fe3d:text2d_get_size_x")
	{
		...
		if(_validateArgumentCount(args, static_cast<unsigned int>(types.size())) && _validateArgumentTypes(args, types))
		{
			if(_validateFe3dText2d(args[0]->getString(), false))
			{
				const auto result = Tools::convertSizeRelativeFromViewport(_fe3d->text2d_getSize(args[0]->getString())).x;
	...
	else if(functionName == "fe3d:text2d_get_size_y")
	{
		...
		if(_validateArgumentCount(args, static_cast<unsigned int>(types.size())) && _validateArgumentTypes(args, types))
		{
			if(_validateFe3dText2d(args[0]->getString(), false))
			{
				const auto result = Tools::convertSizeRelativeFromViewport(_fe3d->text2d_getSize(args[0]->getString())).y;
