...
void WorldEditor::clearLoadedWorld()
{
	...
	for(const auto & [placedId, templateId] : _loadedTextIds)
	{
...
void WorldEditor::_handleInputBox(const string & screenId, const string & leftButtonId, const string & inputBoxId, const string & rightButtonId, float & value, float delta, float multiplier, float minimum, float maximum)
{
	...
	{
		if(_gui->getRightViewport()->getWindow("main")->getScreen(screenId)->getButton(leftButtonId)->isHovered() && _fe3d->input_isMouseHeld(MouseButton::BUTTON_LEFT))
		{
			value = clamp((value - delta), minimum, maximum);
		}
		if(_gui->getRightViewport()->getWindow("main")->getScreen(screenId)->getButton(rightButtonId)->isHovered() && _fe3d->input_isMouseHeld(MouseButton::BUTTON_LEFT))
		{
			value = clamp((value + delta), minimum, maximum);
		}
