...
void WorldEditor::_updateQuad3dPlacing()
{
	if(!_currentEditorQuad3dId.empty())
	{
		...
		{
			const auto newPosition = _fe3d->quad3d_getPosition(_currentEditorQuad3dId);
			...
			if((_gui->getOverlay()->getValueFormId() == "positionX") && _gui->getOverlay()->isValueFormConfirmed())
			{
				...
				_fe3d->quad3d_setPosition(_currentEditorQuad3dId, fvec3(value, newPosition.y, newPosition.z));
			}
			else if((_gui->getOverlay()->getValueFormId() == "positionY") && _gui->getOverlay()->isValueFormConfirmed())
			{
				...
				_fe3d->quad3d_setPosition(_currentEditorQuad3dId, fvec3(newPosition.x, value, newPosition.z));
			}
			else if((_gui->getOverlay()->getValueFormId() == "positionZ") && _gui->getOverlay()->isValueFormConfirmed())
			{
				const auto content = _gui->getOverlay()->getValueFormContent();
				const auto value = (Tools::isInteger(content) ? static_cast<float>(Tools::parseInteger(content)) : 0.0f);
				const auto newQuad3dId = (_currentEditorQuad3dId.substr(1) + "_" + to_string(_quad3dIdCounter));

				_quad3dIdCounter++;

				_duplicator->copyEditorQuad3d(newQuad3dId, _currentEditorQuad3dId);
		...
		else
		{
			...
			_fe3d->quad3d_setPosition(_currentEditorQuad3dId, newPosition);
