...
const bool WorldEditor::saveEditorWorldToFile()
{
	...
	for(const auto& modelID : _fe3d.model_getAllIDs())
	{
		...
		if((modelID[0] != '@') || isLevelOfDetailEntity)
		{
			...
			if(!_meshAnimationEditor.getStartedAnimationIDs(modelID).empty())
			{
			...
			auto animationID = (_meshAnimationEditor.getStartedAnimationIDs(modelID).empty()) ? "" :
				_meshAnimationEditor.getStartedAnimationIDs(modelID)[0];
